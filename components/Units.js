...
function getDisplayText(physicalQuantity, value, precision, unitMatchValue = undefined) {
	...
	case V.Enums.Units_PhysicalQuantity_Power:
		if (isNaN(value)) {
			unitText = "W"
		} else {
		...
		break;
	case V.Enums.Units_PhysicalQuantity_Voltage:
		if (isNaN(value)) {
			unitText = "V"
		} else {
			value = _adjustedValue(value, precision, unitMatchValue)
		...
		break;
	case V.Enums.Units_PhysicalQuantity_Current:
		if (isNaN(value)) {
			unitText = "A"
		} else {
			value = _adjustedValue(value, precision, unitMatchValue)
		...
		break;
	case V.Enums.Units_PhysicalQuantity_Percentage:
		value = isNaN(value) ? value : Math.round(value)
		unitText = "%"
		break;
	case V.Enums.Units_PhysicalQuantity_Temperature:
		unitText = "\u00b0"
		break;
	case V.Enums.Units_PhysicalQuantity_Liters:
		if (isNaN(value)) {
			unitText = "\u2113" // 'l' symbol
		} else {
			value = _adjustedValue(value, precision, unitMatchValue)
		...
		break;
	case V.Enums.Units_PhysicalQuantity_CubicMeters:
		value = isNaN(value) ? value : value.toPrecision(precision)
		unitText = "\u33A5"
		break;
	case V.Enums.Units_PhysicalQuantity_Gallons:
		unitText = "gal"
		break;
	default:
		break;
