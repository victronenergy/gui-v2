...
/*
	Allows a dialog to be vertically dragged downwards off screen.

	This is only used in portrait layout.
*/
MouseArea {
	id: root
	...
	states: State {
		name: "dragging"
		when: root.dialog && (root._readyToDrag
				// maintain the fixed height and shadow increase while the dialog is animating to
				// its destination position.
				|| pushOffscreenAnim.running || restoreYAnim.running)

		// Fix the dialog height; otherwise, it will shrink when dragged.
		PropertyChanges {
			target: root.dialog
			height: root._dialogHeightOnPress
		}

		// The shadow moves along with the dialog, so increase the shadow size while dragging,
		// otherwise the edge of the shadow is revealed.
		PropertyChanges {
			target: root.shadow
	...
	onMouseYChanged: {
		if (_readyToDrag
				// Prevent upwards dragging over a small threshold.
				&& root.dialog.y > _dialogYOnPress - Theme.geometry_modalDialog_grabber_bottomDragLimit) {
