...
QtObject {
	...
	readonly property bool checkingForUpdate: root.updateCheckTimer.running || root.asyncValueTimer.running
	...
	property Timer updateCheckTimer: Timer {
		repeat: false
		running: false
	...
	property Timer asyncValueTimer: Timer {
		repeat: false
		running: false
	...
	property VeQuickItem _stateItem: VeQuickItem {
		...
		onValueChanged: {
			...
			case FirmwareUpdater.UpdateFileNotFound:
				...
				if (root.updateCheckTimer.running) {
					root.asyncValueTimer.start()
					root.updateCheckTimer.stop()
				}
				break
	...
	function maybeFinishUpdateCheck(updateType, valid) {
		if (_updateType === updateType && root.updateCheckTimer.running) {
			if (valid) {
				root.updateCheckTimer.stop()
				Qt.callLater(root._finishUpdateCheck)
			} else {
	...
	function checkForUpdate(updateType) {
		...
		root.asyncValueTimer.stop()
		root.updateCheckTimer.start()
