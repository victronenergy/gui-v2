...
Rectangle {
	...
	readonly property real _nextPos: (width !== Infinity && height !== Infinity) ? orientation === Qt.Vertical ? height - (height * _value)    // slide in from bottom to top
	 : -width + (width * _value)     // slide in from left to right
	 : 0

	on_NextPosChanged: if (visible)
		_updateGauge()
	...
	XAnimator {
		...
		onRunningChanged: if (!running)
			Qt.callLater(_updateGauge) // if _nextPos changed during previous animation
	}
	YAnimator {
		id: yAnimator
		target: fgRect
