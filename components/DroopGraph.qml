...
Item {
    ...
    QuantityLabel {
        ...
        property real _yAdjustment: Math.abs(_yCentre) > _yLabelHeight/2
                                    ? _yLabelHeight/2
                                    : _yLabelHeight/2
    ...
    QuantityLabel {
        ...
        // This calculates the centre value between the two guidelines
        // and then sets a +/- adjustment to be applied to the two labels
        property real _xLabelWidth: Math.max(width, xAxisOperationQuantityLabel.width) + 4 // Padding between labels
        property real _xCentre: (_guidelineIntersection.x + _indicator.x)/2
        property bool _xUseDefaultValues: Math.abs(_guidelineIntersection.x - _indicator.x) + 4 > _xLabelWidth/2
        property real _xAdjustment: _xUseDefaultValues
                                    ? _xLabelWidth/2
                                    : _xLabelWidth/2
                                       - Math.abs(_guidelineIntersection.x - _indicator.x)/2

        anchors.bottom: parent.bottom
        x: _xUseDefaultValues ? _guidelineIntersection.x - width/2
            : _xCentre + _xAdjustment * Math.sign(_guidelineIntersection.x - _indicator.x)
    ...
    QuantityLabel {
        ...
        anchors.bottom: parent.bottom
        x: xAxisReferenceQuantityLabel._xUseDefaultValues ? _indicator.x - width/2
            : xAxisReferenceQuantityLabel._xCentre - xAxisReferenceQuantityLabel._xAdjustment * Math.sign(_guidelineIntersection.x - _indicator.x)
