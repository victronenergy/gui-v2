...
BaseListView {
	...
	delegate: BaseListItem {
		...
		property bool lastListItem: index === root.model.length - 1

		height: root.height
		width: root.totalDelegateWidth / root.model.length
		...
		Button {
			...
			anchors.rightMargin: lastListItem ? Theme.geometry_button_border_width : 0

			radius: 0 // this influences the press effect
			topLeftRadius: 0
			bottomLeftRadius: 0
			...
			backgroundColor: root.enabled ? index === root.currentIndex ? root.checked ? Theme.color_ok : Theme.color_button_off_background : Theme.color_darkOk : Theme.color_background_disabled
