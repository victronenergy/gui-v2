...
OverviewWidget {
	...
	onClicked: {
		if (Services.evcs.model.count === 1) {
			Global.pageManager.pushPage("/pages/evcs/EvChargerPage.qml",
					{ bindPrefix: Services.evcs.model.firstObject.serviceUid })
		} else {
	...
	contentItem: GridLayout {
		...
		Loader {
			...
			sourceComponent: Services.evcs.model.count > 1 ? multiEvChargerComponent
					: Services.evcs.model.count > 0 && Services.evcs.model.firstObject ? singleEvChargerComponent
					: null
			Layout.fillWidth: true
	...
	Component {
		...
		Column {
			...
			readonly property string serviceUid: Services.evcs.model.firstObject.serviceUid
			...
			Label {
				...
				text: Services.evcs.chargerStatusToText(statusItem.value)
			...
			Row {
				...
				Label {
					...
					elide: Text.ElideRight
					text: Services.evcs.chargerModeToText(modeItem.value)
					color: Theme.color_font_secondary
					font.pixelSize: root.tertiaryFontSize
			...
			Repeater {
				...
				delegate: Row {
					...
					Label {
						...
						elide: Text.ElideRight
						text: Services.evcs.chargerStatusToText(model.status)
						font.pixelSize: root.tertiaryFontSize
						color: Theme.color_font_secondary
					}
