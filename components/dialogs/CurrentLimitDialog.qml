...
NumberSelectorDialog {
	...
	decimals: value <= 25 ? 1 : 0
	stepSize: value <= 25 ? 0.1 : 1.0
	...
	onAboutToIncrease: {
		if (value === 25.0) {
			...
			// instead, we want to increase the step size to 1.0 and then reinstate the binding
			// after the spinbox has increased the value by that step size.
			root.stepSize = 1.0
			Qt.callLater(function() { root.stepSize = Qt.binding(function() { return value <= 25 ? 0.1 : 1.0 }) })
		}
	}
