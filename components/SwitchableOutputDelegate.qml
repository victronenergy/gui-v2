		...
		MultiStepButton {
			...
			function generateModel() {
				if (multiStepMax.value === undefined) {
					...
					let totalOptions = Math.min(10, multiStepMax.value)
					for (let i = 0; i < totalOptions; i++) {
						items.push({ 'text': i, 'backgroundColor': i === multiStep.currentIndex ? Theme.color_ok : Theme.color_gray1 })
					}
			...
			width: root._buttonWidth
			height: Theme.geometry_switchableoutput_button_height
			currentIndex: multiStepSelection.valid ?? multiStepSelection.value
			checked: multiStepState.backendValue
			onClicked: (index) => { multiStepSelection.setValue(index) }
			onToggled: multiStepState.writeValue(!multiStepState.backendValue)
			...
			SettingSync {
				...
				backendValue: output.state
				onUpdateToBackend: (value) => { output.setState(value) }
				onBackendValueChanged: multiStep.generateModel()
			}
