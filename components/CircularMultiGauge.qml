		...
		Repeater {
			...
			delegate: Row {
				...
				Label {
					...
					states: State {
						when: Global.systemSettings.briefView.units !== VenusOS.BriefView_Unit_None
						PropertyChanges {
							target: valueLabel
							...
							unit: {
								if (Global.systemSettings.briefView.units === VenusOS.BriefView_Unit_Percentage) {
									return VenusOs.Units_Percentage
								} else if (model.tankType === VenusOS.Tank_Type_Battery) {
									return VenusOS.Units_AmpHour
								} else {
