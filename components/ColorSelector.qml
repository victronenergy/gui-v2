...
Item {
	...
	function resetHue() {
		mousearea.angle = ((root.colorDimmerData.color.hslHue * 360 + 360) - (90 - colorGradient.angle)) % 360
	}
		...
		MouseArea {
			...
			function updateAngle() {
				if (Math.sqrt(Math.pow(width/2 - mouseX, 2) + Math.pow(mouseY - height/2, 2)) > (Theme.geometry_colorWheel_selector_centre_diameter/2 + Theme.geometry_colorWheel_selector_centre_borderWidth/2) &&
					...
					angle = Math.round((Math.atan2(width/2 - mouseX, mouseY - height/2) / 6.2831 + 0.5) * 360)
				}
			}
