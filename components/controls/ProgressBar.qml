...
T.ProgressBar {
	...
	background: Rectangle {
		...
		Rectangle {
			...
			Binding {
				when: !indeterminateAnim.running
				highlightRect.width: root.indeterminate ? 0 : root.availableWidth * root.visualPosition
			}
			...
			SequentialAnimation {
				...
				// Use an animator instead of a NumberAnimation, so that part of the animation does
				// not run on the main thread when many objects are being loaded.
				XAnimator {
					target: highlightRect
