...
Column {
	...
	function _trackerHistoryTotal(role, trackerIndex) {
		...
		for (let day = dayRange[0]; day < dayRange[1]; ++day) {
			const history = trackerIndex === undefined ? root.solarHistory.dailyHistory(day) : root.solarHistory.dailyTrackerHistory(day, trackerIndex);
			if (history) {
	...
	Item {
		width: 1
		height: isNaN(root.minimumHeight) ? 0 : root.minimumHeight - tableSummary.height - trackerTable.height - solarDetailBox.height
	}
