...
ListModel {
	...
	function _reset() {
		...
		if (gauges !== _gaugeObjects.model) {
			...
			_batteriesItem.active = hasBatteryId
			_gaugeObjects.model = gauges
			...
			_tankCountConn.enabled = useDefaultGauges
		}
	}
	...
	readonly property Instantiator _gaugeObjects: Instantiator {
		...
		delegate: Loader {
			...
			Component {
				...
				GaugeSource {
					readonly property string _serviceUid: gaugeObject.modelData.value
					readonly property TankModel _tankModel: type >= 0 ? Global.tanks.tankModel(type) : null
					readonly property Tank _device: _tankModel ? _tankModel.deviceAt(_tankModel.indexOf(_serviceUid)) : null

					readonly property VeQuickItem _fluidType: VeQuickItem {
						uid: _serviceUid + "/FluidType"
					}

					name: _device?.name || ""
					type: _fluidType.value ?? -1
