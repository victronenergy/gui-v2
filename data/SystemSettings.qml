	...
	property QtObject briefView: QtObject {
		...
		function _refreshCentralGauges() {
			...
			for (let i = 0; i < Theme.geometry_briefPage_centerGauge_maximumGaugeCount; ++i) {
				...
				// - string: the service uid (to show the level for that particular tank) or a
				...
				if (!obj || !obj.isValid || obj.value === "") {
					continue
				}
				let centerGaugeType
				const tankType = parseInt(obj.value)
				if (isNaN(tankType)) {
					centerGaugeType = BackendConnection.serviceTypeFromUid(obj.value) === "tank"
							? VenusOS.BriefView_CentralGauge_TankUid
