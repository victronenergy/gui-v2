...
QtObject {
	...
	function timeToGoText(battery, format) {
		...
		if (format === Enums.Battery_TimeToGo_LongFormat) {
	...
	function batteryMode(battery) {
		return isNaN(battery.power) || battery.power === 0 ? Enums.Battery_Mode_Idle
			: (battery.power > 0 ? Enums.Battery_Mode_Charging : Enums.Battery_Mode_Discharging)
	}

	function modeToText(mode) {
		switch (mode) {
		case Enums.Battery_Mode_Idle:
			return CommonWords.idle
		case Enums.Battery_Mode_Charging:
			return CommonWords.charging
		case Enums.Battery_Mode_Discharging:
