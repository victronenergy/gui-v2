...
Device {
	...
	readonly property ObjectAcConnection _phaseMeasurements: ObjectAcConnection {
		...
		bindPrefix: {
			...
			case "vebus":
				...
				if (root.inputInfo.connected && _activeInput.isValid && root.inputInfo.inputIndex === _activeInput.value) {
	...
	readonly property VeQuickItem _activeInput: VeQuickItem {
		uid: root.inputInfo && root.inputInfo.valid && (root.serviceType === "vebus" || root.serviceType === "acsystem") ? root.serviceUid + "/Ac/ActiveIn/ActiveInput" : ""
	}
