...
/*
	Sets battery values for the system and settings services.

	Services with /Dc/[0-9] are assumed to be batteries, and are added to:
	...
	If the active battery monitor and active BMS service are not set, and the settings is configured to
	allow the system to auto-select it, then:
...
Item {
	...
	// Services of these types are not treated as batteries, even if they have a /Dc/[0-9] value.
	readonly property var excludedServiceTypes: ["solarcharger"]
	...
	QtObject {
		...
		function removeBattery(battery) {
			...
			for (const path of ["/Enabled", "/Name", "/Service"]) {
				MockManager.clearValue(`{$prefix}/path`)
			}
		}
	}

	// Find Battery entries on the system: assume any service with a /Dc/<0-9> path contains a
	// battery. Ignore entries from solarcharger services.
	Instantiator {
		model: VeQItemSortTableModel {
			dynamicSortFilter: true
			filterRole: VeQItemTableModel.UniqueIdRole
			filterFlags: VeQItemSortTableModel.FilterOffline
			filterRegExp: "^mock/com\.victronenergy\.\\w+\.\\w+\/Dc\/\\d+$"
		...
		delegate: Device {
			...
			// Channel comes from /Dc/<channel>. Normally the channel 0, unless there are multiple
			// batteries on the service.
			property int channel: parseInt(id)

			property bool syncChanges

			function init() {
				// If there is a channel, check there is also a /Devices entry to get other
				// metadata. If not, ignore it and do not add it to the battery configurations.
				if (channel > 0) {
					_customName.uid = `${serviceUid}/Devices/${channel}/CustomName`
					_productName.uid = `${serviceUid}/Devices/${channel}/ProductName`
			...
			component BatteryProperty: VeQuickItem {
				...
				uid: path ? `${battery.serviceUid}${path}` : ""
		...
		onObjectAdded: (index, battery) => {
			if (root.excludedServiceTypes.indexOf(BackendConnection.serviceTypeFromUid(battery.serviceUid)) >= 0) {
				return
			}
			if (!battery.init()) {
				return
			...
			battery.syncChanges = true
		}
		onObjectRemoved: (index, battery) => {
			if (battery.syncChanges) {
				battery.syncChanges = false
