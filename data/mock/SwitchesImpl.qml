		...
		SwitchDev {
			...
			property Timer _measurementUpdates: Timer {
				...
				onTriggered: {
					for (let i = 0; i < switchDev.inputCount; ++i) {
						if (i != 1){
							switchDev.setMockValue("/Channel/%1/Voltage".arg(i), Math.random() * 50)
							switchDev.setMockValue("/Channel/%1/Current".arg(i), Math.random() * 10)
							switchDev.setMockValue("/Channel/%1/Current".arg(i), Math.random() * 10)
						}
					}
					...
					readonly property VeQuickItem switchState: VeQuickItem {
						...
						onValueChanged: {
							if (value){
								...
								if (index === 0 ){
									switchDev.setMockValue("/Channel/%1/Status".arg(index),  statusData[ _testStatusIndex])
									_testStatusIndex++
									if (_testStatusIndex >= statusData.length) _testStatusIndex = 0
								} else switchDev.setMockValue("/Channel/%1/Status".arg(index), VenusOS.Switch_Status_On)

							} else {
								switchDev.setMockValue("/Channel/%1/Status".arg(index), VenusOS.Switch_Status_Off)
							}
						}
					}
					readonly property VeQuickItem switchStatus: VeQuickItem {
						uid: switchDev.serviceUid + "/Channel/%1/Status".arg(index)
					}
				}
			}
			...
			Component.onCompleted: {
				...
				for (let i=0;i<switchDev.inputCount;i++){
					if (i === 1) switchDev.setMockValue("/Channel/%1/CustomName".arg(i), "cust %1".arg(i))
					else switchDev.setMockValue("/Channel/%1/CustomName".arg(i),"")
					switchDev.setMockValue("/Channel/%1/Function".arg(i), i % 3)
					switchDev.setMockValue("/Channel/%1/State".arg(i), 0)
					switchDev.setMockValue("/Channel/%1/Status".arg(i), 0)
					//optional
					if (i != 0) switchDev.setMockValue("/Channel/%1/Dimming".arg(i), 50)
					if (i != 2) switchDev.setMockValue("/Channel/%1/Fuse".arg(i), 5)
					...
					if (i <2) switchDev.setMockValue("/Channel/%1/GroupName".arg(i), "")
					else switchDev.setMockValue("/Channel/%1/GroupName".arg(i), "Group %1".arg(Math.round(i/2)))
				}
			}
		}
	}
