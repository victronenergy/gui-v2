...
Q_SIGNALS:
	...
	void themeValueChanged(ThemeBase::ScreenSize newValue);

public Q_SLOTS:
	void handleThemeValueChanged(ThemeBase::ScreenSize newValue) {
...
bool ThemeParser::load(const QString &themeDir, bool generateFlatThemeObject)
{
	...
	for (int idx = 0; idx < screenSizeMetaEnum.keyCount(); ++idx) {
		...
		themeInfo.ternaryCondition = QStringLiteral("%1 == ThemeBase::%2 ?").arg(
