...
void main() {
    ...
    float adjustedAngle = (uvAngle > 0.92 && ubuf.shineAngle < 0.08 && startCapAlpha > 0.0) ? (uvAngle - 1.0) : uvAngle;
    float shineMix = smoothstep(ubuf.shineAngle - 0.08, ubuf.shineAngle, adjustedAngle)
                     * (1.0 - smoothstep(ubuf.shineAngle, ubuf.shineAngle + 0.02, adjustedAngle));
    ...
    float isProgressBar = ((startCapAlpha > 0.0 && ubuf.progressAngle > 0.0)
                           || (uvAngle <= ubuf.progressAngle)) ? 1.0 : 0.0;
