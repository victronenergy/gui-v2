...
if [[ -n "${HOST_LIST}" ]]; then
    ...
    for H in "${HOSTS[@]}"; do
        ...
        echo "Processing host: ${HOST} on port ${PORT}"
        ...
        rsync -az --info=progress2 -e "ssh -p ${PORT}" "${FILES_DIR}/wasm/" root@${HOST}:/var/www/venus/gui-v2
        if [ $? -ne 0 ]; then
        ...
        ssh -p ${PORT} root@${HOST} "svc -t /service/vrmlogger"
        echo " done."
        echo
    done
fi
