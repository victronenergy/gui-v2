...
Item {
	...
	Connections {
		target: Global.pageManager && Global.pageManager.emitter ? Global.pageManager.emitter : null

		function onDemoKeyPressed(event) {
			switch (event.key) {
			case Qt.Key_Left:
				if (Global.pageManager.navBar.currentUrl in root._demos) {
					previousConfig()
					event.accepted = true
				}
				break
	...
	Connections {
		target: Global.pageManager && Global.pageManager.navBar ?  Global.pageManager.navBar : null
