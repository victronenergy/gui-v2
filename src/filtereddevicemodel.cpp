...
private:
	...
	void removeFilteredChild(VeQItem *child);
	...
	QMap<QString, QPointer<VeQItem> > m_childItems;
	...
	, m_model(model)
{
	...
	connect(AllDevicesModel::create(), &AllDevicesModel::deviceAdded, this, [this](Device *device) {
		addChildrenOfDevice(device, true);
	});
	connect(AllDevicesModel::create(), &AllDevicesModel::deviceAboutToBeRemoved,
...
void ChildFilterHandler::addChildrenOfDevice(Device *device, bool initialized)
{
	...
	if (auto it = m_filterIds.find(device->serviceType()); it != m_filterIds.end()) {
		...
		// removal, or added if they do not yet exit.
		...
		connect(device->serviceItem(), &VeQItem::childAboutToBeRemoved,
				this, &ChildFilterHandler::removeFilteredChild,
				Qt::UniqueConnection);
	}
}
...
void ChildFilterHandler::removeChildrenOfDevice(Device *device)
{
	...
	if (device
		...
		for (auto it = m_childItems.begin(); it != m_childItems.end();) {
			if (it.value() && it.value()->itemParent() == device->serviceItem()) {
...
void ChildFilterHandler::removeFilteredChild(VeQItem *child)
{
	...
	: QSortFilterProxyModel(parent)
{
	...
	connect(this, &FilteredDeviceModel::modelReset, this, &FilteredDeviceModel::updateCount);
...
BaseDevice *FilteredDeviceModel::firstObject() const
{
	return deviceAt(0);
}
...
void FilteredDeviceModel::updateCount()
{
	...
	const QString firstUid = firstDevice ? firstDevice->serviceUid() : QString();
	if (m_firstUid != firstUid) {
		m_firstUid = firstUid;
