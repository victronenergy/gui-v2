...
void WifiModel::update()
{
	...
	*/
	...
	for (int i = m_networks.count() - 1; i >= 0; --i) {
		if (!wifis.contains(m_networks.at(i).service)) {
	...
	for (auto it = wifis.constBegin(); it != wifis.constEnd(); ++it) {
		...
		const bool favourite = details.value(QStringLiteral("Favorite")).toString() == QStringLiteral("yes");
		bool found = false;
		...
		for (int j = 0; j < m_networks.count(); ++j) {
			if (!service.isEmpty() && service == m_networks.at(j).service) {
				...
				if (m_networks[j].favorite != favourite) {
					m_networks[j].favorite = favourite;
