	...
	: QAbstractListModel(parent)
{
	...
	m_roleNames.insert(NotificationRoles::Silenced, "silenced");
...
QVariant NotificationsModel::data(const QModelIndex &index, int role) const
{
	...
	case Silenced:
		return m_data.at(row).get()->silenced();
	case Active:
...
void NotificationsModel::insert(const int index, BaseNotification* notification)
{
	...
	connect(notification, &BaseNotification::silencedChanged,       this, &NotificationsModel::silencedChangedHandler);
...
void NotificationsModel::silencedChangedHandler()
{
	BaseNotification * notification = qobject_cast<BaseNotification*>(sender());
	roleChangedHandler(notification, NotificationRoles::Silenced);
}
