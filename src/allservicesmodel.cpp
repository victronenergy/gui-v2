	...
	: QAbstractListModel(parent)
{
	...
	connect(BackendConnection::instance(), &BackendConnection::producerChanged,
...
{
	if (!BackendConnection::instance()->producer()
			|| !BackendConnection::instance()->producer()->services()) {
		clear();
		return;
	}

	VeQItem *servicesRoot = BackendConnection::instance()->producer()->services();
	if (BackendConnection::instance()->type() == BackendConnection::MqttSource) {
...
void AllServicesModel::serviceItemDiscovered(VeQItem *serviceItem)
{
	...
	m_services.append({ BackendConnection::instance()->serviceTypeFromUid(serviceItem->uniqueId()), serviceItem });
	endInsertRows();
	emit countChanged();    
...
void AllServicesModel::addServicesFromChildrenOf(VeQItem *parentItem)
{
	...
	if (services.count() > 0) {
		...
		for (auto it = services.constBegin(); it != services.constEnd(); ++it) {
			m_services.append({ BackendConnection::instance()->serviceTypeFromUid(it.value()->uniqueId()), it.value() });
		}
		endInsertRows();
		emit countChanged();
	}
...
AllServicesModel* AllServicesModel::instance()
{
	return create();
}

AllServicesModel* AllServicesModel::create(QQmlEngine *, QJSEngine *)
{
	static AllServicesModel* instance = new AllServicesModel;
	return instance;
}
