...
/*
	...
	The model can be filtered to only show devices with specific child values, using a custom
	filtering function. For example, the model below includes generator, vebus and tank devices on
	...
	FilteredDeviceModel {
		...
		// For generators, we will check the value of com.victronenergy.generator.<suffix>/Enabled
		// and com.victronenergy.generator.<suffix>/Active. For vebus services, we will check the
		// value of com.victronenergy.vebus.<suffix>/DeviceInstance.
		childFilterIds: { "generator": ["Enabled", "Active"], "vebus": ["DeviceInstance"] }

		// Provide a custom filter function. For each device in the model, if its service type is
		// in the childFilterIds map, then this function is called with a {childId: VeQItems}
		// map of the requested childItems for that service type.
		childFilterFunction: (device, childItems) => {
			...
			// This code should not be reached; the function is not called for tanks, as there are
			// no child id filters set for tank types.
			return false
		}
	}

	Note: the child ids used in the filter must be direct children. "ChildValue" is supported, but
	not "Some/Deeper/ChildValue".
*/
