...
namespace {
	QString customisationsDirPath() {
#if defined(VENUS_WEBASSEMBLY_BUILD)
		return QString();
#elif defined(VENUS_GX_BUILD)
		// look in /tmp/venus-gui-v2/customisations/
		const QString path = QStringLiteral("/tmp/venus-gui-v2/customisations/");
		QDir dir;
		if (!dir.exists(path)) {
			if (!dir.mkpath(path)) {
				qCWarning(venusGui) << "/tmp/venus-gui-v2/customisations/ does not exist and could not be created!";
				return QString();
			}
		}
		return path;
		...
		QDir customisationsDir = QDir(appDir);
		if (!customisationsDir.exists(QStringLiteral("customisations"))
				&& !customisationsDir.mkdir(QStringLiteral("customisations"))) {
			qCWarning(venusGui) << QStringLiteral("%1/%2").arg(appDir, QStringLiteral("customisations"))
				<< "does not exist and could not be created!";
			return QString();
		}
		if (customisationsDir.cd(QStringLiteral("customisations"))) {
			return customisationsDir.canonicalPath();
		}
		return QString();
#else
		return QString();
...
#if !defined(VENUS_WEBASSEMBLY_BUILD)
	const QString path = customisationsDirPath();
	if (!path.isEmpty()) {
		qCInfo(venusGui) << "Attempting to load customisations from filesystem path:" << path;
		QFileSystemWatcher *watcher = new QFileSystemWatcher(QStringList { path }, this);
		connect(watcher, &QFileSystemWatcher::directoryChanged,
			this, [this, path] {
				setCustomisationsJson(loadFromFilesystem(path));
			});
		setCustomisationsJson(loadFromFilesystem(path));
	}
#endif
...
QString Customisations::loadFromFilesystem(const QString &path) const
{
	QDir customisationsDir(path);
	const QStringList files = customisationsDir.exists()
		? customisationsDir.entryList({ QStringLiteral("*.json") }, QDir::Files)
		: QStringList();
...
void Customisations::populateCustomisations()
{
	...
	qCInfo(venusGui) << "Populating customisations from array with length" << array.size();
	...
	for (qsizetype i = 0; i < array.size(); ++i) {
		...
		qCInfo(venusGui) << "Successfully populated customisation at index" << i << ":" << customisationName;
		data.append(c);
	}
