...
QVariant RuntimeDeviceModel::data(const QModelIndex &index, int role) const
{
	...
	case ConnectedRole:
		return !m_devices.at(row).device.isNull();
	default:
		return QVariant();
	}
}
...
void RuntimeDeviceModel::removeDisconnectedDevices()
{
	...
	for (auto it = m_disconnectedDevices.begin(); it != m_disconnectedDevices.end();) {
		...
		if (index >= 0) {
			...
			DeviceInfo info = m_devices.takeAt(index);
			if (info.device) {
	...
	for (int i = first; i <= last; ++i) {
		if (BaseDevice *device = AllDevicesModel::create()->deviceAt(i)) {
			...
			if (deviceIndex < 0) {
				...
				// The device is already in the model; this means it was disconnected, and has now been reconnected.
