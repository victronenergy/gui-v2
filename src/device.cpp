...
void Device::setServiceItem(VeQItem *serviceItem)
{
	...
	if (m_serviceItem) {
		qWarning() << "Service item already set for" << serviceUid() << "but got request to set it";
		return;
	}
...
void Device::serviceChildAdded(VeQItem *child)
{
	if (child->id() == QStringLiteral("DeviceInstance")) {
		...
		m_connections[child->id()] = connect(child, &VeQItem::valueChanged, this, [this](QVariant value) {
	...
	} else if (serviceType() == QStringLiteral("tank") && child->id() == QStringLiteral("FluidType")) {
		const QVariant value = child->getValue();
		if (value.isValid()) {
...
{
	if (auto it = m_connections.find(child->id()); it != m_connections.end()) {
		QObject::disconnect(it.value());
...
void Device::initializeFromServiceUid()
{
	if (!serviceUid().isEmpty() && !m_serviceItem) {
		...
		VeQItem *serviceItem = AllServicesModel::create()->itemAt(AllServicesModel::create()->indexOf(serviceUid()));
		if (serviceItem) {
			setServiceItem(serviceItem);
		} else {
			connect(AllServicesModel::create(), &AllServicesModel::serviceAdded, this, [this](VeQItem *item) {
