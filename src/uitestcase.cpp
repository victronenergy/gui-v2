...
QString UiTestCase::sanitizedImageName(const QString &imageName) const
{
	...
	// Limit to 255 char limit, minus 3 for png file extension.
...
QObject *UiTestCase::doFindObject(QObject *sourceObject, const QVariantMap &params, const QString &typeName, int loggingIndent) const
{
	...
	const QObjectList &children = sourceObject->children();
	QString indentation;
	if (loggingIndent >= 0) {
...
QQuickItem *UiTestCase::doFindClickableChild(QQuickItem *sourceItem, int loggingIndent) const
{
	...
	const QList<QQuickItem *> childItems = sourceItem->childItems();
	QString indentation;
	if (loggingIndent >= 0) {
		indentation.fill(' ', loggingIndent);
	}
