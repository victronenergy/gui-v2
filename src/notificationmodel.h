...
public:
	...
	bool acknowledged() const { return m_acknowledged ? m_acknowledged->getValue().toBool() : false; }
	bool active() const { return m_active ? m_active->getValue().toBool() : false; }
	bool silenced() const { return m_silenced ? m_silenced->getValue().toBool() : false; }
