...
/*
	...
	If serviceUid is set, then the model only shows the history for that service. (If the service
	has multiple trackers, the model will include all of them.) Otherwise the model includes the
	yield for all solarcharger, multi and inverter services on the system with /History values.
*/
...
private:
	...
	bool shouldMonitorDevice(Device *device) const;
