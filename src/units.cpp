...
{
    if (formatHints == Units::FormatHint::CompactUnitFormat
        && (unit == Victron::VenusOS::Enums::Units_Temperature_Celsius
            || unit == Victron::VenusOS::Enums::Units_Temperature_Fahrenheit
            || unit == Victron::VenusOS::Enums::Units_Temperature_Kelvin)) {
        return QString(UnitTable[unit].label).first(1);
    }
