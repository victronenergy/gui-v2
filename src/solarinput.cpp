...
{
	if (!device) {
		qmlWarning(this) << "invalid device!";
		return;
	}
	if (VeQItem *serviceItem = device->serviceItem()) {
		if (isSingleTrackerDevice) {
			m_todaysYieldItem = serviceItem->itemGet(QStringLiteral("History/Daily/0/Yield"));
			m_powerItem = serviceItem->itemGet(QStringLiteral("Yield/Power"));
			m_voltageItem = serviceItem->itemGet(QStringLiteral("Pv/V"));
		} else {
			m_todaysYieldItem = serviceItem->itemGet(QStringLiteral("History/Daily/0/Pv/%1/Yield").arg(trackerIndex));
			m_powerItem = serviceItem->itemGet(QStringLiteral("Pv/%1/P").arg(trackerIndex));
			m_voltageItem = serviceItem->itemGet(QStringLiteral("Pv/%1/V").arg(trackerIndex));
		}
		m_nameItem = serviceItem->itemGet(QStringLiteral("Pv/%1/Name").arg(trackerIndex));
		m_enabledItem = serviceItem->itemGet(QStringLiteral("Pv/%1/Enabled").arg(trackerIndex));
		...
		if (m_todaysYieldItem) {
			setTodaysYield(m_todaysYieldItem->getValue());
...
{
	// Phases are L1, L2, L3
	return childId.startsWith('L') && childId.mid(1).toInt() > 0;
}
...
void PvInverterSolarInput::addPhase(VeQItem *phaseItem)
{
	...
	if (VeQItem *currentItem = phaseItem->itemGet(QStringLiteral("Current"))) {
	...
	if (VeQItem *voltageItem = phaseItem->itemGet(QStringLiteral("Voltage"))) {
...
void PvInverterSolarInput::removePhase(VeQItem *phaseItem)
{
	if (!phaseItem) {
		return;
	}
	m_phases.remove(phaseItem->id());
}
...
void PvInverterSolarInput::updateSinglePhaseValues()
{
	...
	for (const AcPhase &phase : m_phases) {
