	...
	, m_isSingleTrackerDevice(isSingleTrackerDevice)
{
	...
	if (VeQItem *serviceItem = device->serviceItem()) {
		VeQItem *daysAvailableItem = serviceItem->itemGet(QStringLiteral("/History/Overall/DaysAvailable"));
		if (isSingleTrackerDevice) {
			if (daysAvailableItem->getValue().toInt() > 0) {
				m_todaysYieldItem = serviceItem->itemGetOrCreate(QStringLiteral("History/Daily/0/Yield"));
			}
			...
			if (daysAvailableItem->getValue().toInt() > 0) {
				m_todaysYieldItem = serviceItem->itemGetOrCreate(QStringLiteral("History/Daily/0/Pv/%1/Yield").arg(trackerIndex));
			}
		...
		// Note: Name and Enabled items may not be present on older devices.
