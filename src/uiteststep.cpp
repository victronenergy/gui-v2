...
void UiTestStepGroup::addStep(UiTestStep *step)
{
	if (m_active) {
		...
		// instead.
		qCFatal(venusGuiTest) << qPrintable(QStringLiteral("Do not add steps while steps are running, the new steps will not be run"));
	}
	m_steps.append(step);
}
...
CaptureAndCompareStep::CaptureAndCompareStep(QObject *parent, QQuickWindow *window, const QString &filePrefix, const QString &message)
	...
	, m_failureThreshold(settingValue(QStringLiteral("FailureThreshold")).value<qreal>())
{
}
...
QString CaptureAndCompareStep::summary() const
{
	...
	if (m_stabilizationAttempts > maxStabilizationAttempts()) {
		s += QStringLiteral(" (image is unstable?)");
	} else if (m_stabilizationAttempts > 2) { // disregard initial capture verifications
		s += QStringLiteral(" (%1 captures to stabilize)").arg(m_stabilizationAttempts - 2);
	}
	return s;
}
...
void CaptureAndCompareStep::start()
{
	...
	capture();
	const int interval = settingValue(QStringLiteral("CaptureInterval"), 16).toInt();
...
void CaptureAndCompareStep::timerEvent(QTimerEvent *event)
{
	...
	// testing.
	if (prevCapture == m_lastCapture || m_stabilizationAttempts + 1 > maxStabilizationAttempts()) {
...
void CaptureAndCompareStep::finalize()
{
	...
	const QString fileNameExpected = absoluteImagePath(filePath + ".png");
	if (fileNameExpected.isEmpty()) {
		finish(false, QStringLiteral("Image path '%1' is invalid!").arg(fileNameExpected));
		return;
	}

	const QImage expectedImage(fileNameExpected);
	if (expectedImage.isNull()) {
		// No previous version of this image has been stored, so save this captured image as the
		// "expected" image.
		if (!m_lastCapture.save(fileNameExpected)) {
			finish(false, QStringLiteral("Failed to save expected image! %1").arg(fileNameExpected));
			return;
		}
	} else if (!identicalImages(expectedImage, m_lastCapture, m_failureThreshold, absoluteImagePath(filePath + "-DIFF.png"))) {
		...
		finish(false, QStringLiteral("Image comparison failed: %1 != %2").arg(fileNameFailed).arg(fileNameExpected));
		return;
	}
...
QString CaptureAndCompareStep::absoluteImagePath(const QString &fileName) const
{
	static QString dirPath;
	if (dirPath.isEmpty()) {
		dirPath = settingValue(QStringLiteral("ImageDir")).toString();
...
int CaptureAndCompareStep::maxStabilizationAttempts() const
{
	static const int attempts = settingValue(QStringLiteral("MaximumStabilizationAttempts"), 10).toInt();
	return attempts;
}
...
CaptureAndCompareStep *CaptureAndCompareStep::create(QObject *parent, QQuickWindow *window, const QString &imagePrefix, const QVariantMap &params)
{
	...
	return new CaptureAndCompareStep(parent, window, QString("%1-%2")
			.arg(imagePrefix).arg(imageName), messageText(params));
}
