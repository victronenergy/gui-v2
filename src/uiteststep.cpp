...
bool identicalImages(const QImage &img1, const QImage &img2, qreal diffThreshold, const QString &failFileName)
{
	...
	float diffPercent = (static_cast<double>(totalDiff) / 255.0 / totalColourParts) * 100;
...
void UiTestStepGroup::addStep(UiTestStep *step)
{
	if (m_active) {
		...
		// instead.
		qCFatal(venusGuiTest) << "Detected a call to runSteps() while other steps are still running!";
	}
	m_steps.append(step);
}
...
void CaptureAndCompareStep::finalize()
{
	...
	if (QFile::exists(captureFileName)) {
		...
		if (comparisonThreshold.isValid()
			...
			// Comparison failed or is not required, so do not update the saved version of this image.
			return;
		}
	}
