	...
	: QAbstractListModel(parent)
{
	for (int i = 0; i < AllServicesModel::create()->count(); ++i) {
		VeQItem *serviceItem = AllServicesModel::create()->itemAt(i);
	...
	connect(AllServicesModel::create(), &AllServicesModel::modelAboutToBeReset,
...
int AllDevicesModel::indexOf(const QString &uid)
{
	for (int i = 0; i < m_devices.count(); ++i) {
