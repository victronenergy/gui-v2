...
public:
	...
	void append(QQuickItem *item) {
		...
		const QVariant effectiveVisible = item->property("effectiveVisible");
	...
	*/
	int visibleInsertionIndex(QQuickItem *item) {
		...
		for (int i = overallIndex - 1; i >= 0 && i < allItems.count(); --i) {
			...
			const QVariant effectiveVisible = previousItem->property("effectiveVisible");
	...
	QVector<QQuickItem *> visibleItems;
};
