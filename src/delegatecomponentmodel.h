...
/*
	...
	The preferredVisible property is the author-controlled "should this row
	exist" flag (data valid, feature enabled, and so on). showAccessLevel is
	the minimum user access level required to include the row (default User).

	effectiveVisible is what the model actually filters on. By default it is
	preferredVisible combined with the current access level (the same rule as
	SystemSettings.canAccess). A page may bind effectiveVisible to a different
	expression; that binding then replaces the default.

	The model never inspects the incubated delegate. Put access membership on
	the DelegateComponent, not only on the list item:
	...
	When the list item is constructed, DelegateComponentModel sets
	delegateComponent on AbstractListItem so ListSetting can copy
	showAccessLevel (SuperUser stripe) without a per-call-site binding.
...
class DelegateComponent : public QObject
{
	...
	Q_PROPERTY(int showAccessLevel READ showAccessLevel WRITE setShowAccessLevel NOTIFY showAccessLevelChanged FINAL)
...
public:
	...
	int showAccessLevel() const;
	void setShowAccessLevel(int level);

	bool effectiveVisible() const;
	void setEffectiveVisible(bool visible);

Q_SIGNALS:
	void delegateChanged();
	void dataItemChanged();
	void preferredVisibleChanged();
	void showAccessLevelChanged();
	void effectiveVisibleChanged();

private:
	bool computedEffectiveVisible() const;
	void accessLevelPossiblyChanged();

	QQmlComponent *m_delegate = nullptr;
	VeQuickItem *m_dataItem = nullptr;
	bool m_preferredVisible = true;
	int m_showAccessLevel = 0; // Enums::User_AccessType_User
	bool m_effectiveVisible = true;
	bool m_effectiveVisibleExplicit = false;
};
...
/*
	...
	Entries with effectiveVisible=false are excluded from the model entirely:
	no row, no delegate, no object. The incubated list item's effectiveVisible
	is not consulted.
*/
