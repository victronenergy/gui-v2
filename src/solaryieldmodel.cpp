		...
		if (VeQItem *historyItem = overallItem->itemParent()) {  // The /History path
			for (int i = 0; i < m_dailyYields.count(); ++i) {
				...
				if (day < daysAvailable) {
					// History should be available for this day. Call itemGetOrCreate() instead of
					// itemGet(), so that if the yield path is not yet available, yieldValueChanged()
					// is called later when the path is set later with a valid value.
					...
					// No history is available for this day; remove yield item for this device if it
					// was previously added for this day.
