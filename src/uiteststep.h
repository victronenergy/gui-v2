...
protected:
	...
	// Returns a setting (specific to this step type) as read from the test configuration on startup.
	QVariant settingValue(const QString &key, const QVariant &defaultValue = QVariant()) const;

	// Returns a step setting, as read from the test configuration on startup.
