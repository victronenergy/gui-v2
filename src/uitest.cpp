...
void UiTest::loadConfiguration(const QString &relativeTestDir)
{
	...
	if (!file.open(QFile::ReadOnly | QFile::Text)) {
		qCWarning(venusGuiTest) << "Failed to load test configuration! Cannot open file:" << filePath;
		return;
	}
	...
	if (parseError.error != QJsonParseError::NoError) {
		qCWarning(venusGuiTest) << "Failed to parse test configuration! Got parse error:"
