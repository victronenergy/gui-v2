...
class AccessLevelMonitor
{
public:
	static AccessLevelMonitor *instance()
	{
		static AccessLevelMonitor monitor;
		return &monitor;
	}
	...
	bool valid() const
	{
		return m_item.getIsValid();
	}

	int value() const
	{
		return const_cast<VeQuickItem &>(m_item).getValue().toInt();
	}

private:
	AccessLevelMonitor()
	{
		if (BackendConnection *backend = BackendConnection::create()) {
			m_item.setUid(backend->serviceUidForType(QStringLiteral("settings"))
					+ QStringLiteral("/Settings/System/AccessLevel"));
		}
	}
...
DelegateComponent::DelegateComponent(QObject *parent)
	: QObject(parent)
	, m_showAccessLevel(static_cast<int>(Enums::User_AccessType_User))
{
	AccessLevelMonitor *monitor = AccessLevelMonitor::instance();
	connect(&monitor->item(), &VeQuickItem::valueChanged, this, &DelegateComponent::accessLevelPossiblyChanged);
	connect(&monitor->item(), &VeQuickItem::isValidChanged, this, &DelegateComponent::accessLevelPossiblyChanged);
}
...
void DelegateComponent::setPreferredVisible(bool visible)
{
	if (m_preferredVisible != visible) {
		...
		if (!m_effectiveVisibleExplicit) {
			emit effectiveVisibleChanged();
		}
	}
}

int DelegateComponent::showAccessLevel() const
{
	return m_showAccessLevel;
}

void DelegateComponent::setShowAccessLevel(int level)
{
	if (m_showAccessLevel != level) {
		m_showAccessLevel = level;
		emit showAccessLevelChanged();
		if (!m_effectiveVisibleExplicit) {
			emit effectiveVisibleChanged();
		}
	}
...
bool DelegateComponent::effectiveVisible() const
{
	if (m_effectiveVisibleExplicit) {
		return m_effectiveVisible;
	}
	return computedEffectiveVisible();
}

void DelegateComponent::setEffectiveVisible(bool visible)
{
	const bool oldValue = effectiveVisible();
	m_effectiveVisibleExplicit = true;
	m_effectiveVisible = visible;
	if (oldValue != visible) {
		emit effectiveVisibleChanged();
	}
}

bool DelegateComponent::computedEffectiveVisible() const
{
	if (!m_preferredVisible) {
		return false;
	}
	AccessLevelMonitor *monitor = AccessLevelMonitor::instance();
	if (!monitor->valid()) {
		// Until the access level is known, only User-level rows are included. That matches a
		// logged-in User and keeps unit tests (no backend) from hiding every default row.
		return m_showAccessLevel <= static_cast<int>(Enums::User_AccessType_User);
	}
	return monitor->value() >= m_showAccessLevel;
}

void DelegateComponent::accessLevelPossiblyChanged()
{
	if (!m_effectiveVisibleExplicit) {
		emit effectiveVisibleChanged();
	}
...
void DelegateComponentModel::entries_append(QQmlListProperty<DelegateComponent> *prop, DelegateComponent *entry)
{
	...
	QObject::connect(entry, &DelegateComponent::effectiveVisibleChanged, model, [model, entry]() {
...
QObject *DelegateComponentModel::object(int index, QQmlIncubator::IncubationMode)
{
	...
	if (auto *item = qobject_cast<AbstractListItem *>(object)) {
		item->setDelegateComponent(entry);
	} else {
		object->setProperty("delegateComponent", QVariant::fromValue(entry));
	}
	...
	// completeCreate() may have hidden this entry. entryVisibilityChanged() then
	// removes the row but leaves the object because refCount is 1. Do not publish
	// a removed row; the view never received the object so it will not release().
	const int currentIndex = d->visibleIndexOf(entry);
	if (currentIndex < 0) {
		auto objIt = d->objects.find(entry);
		if (objIt != d->objects.end()) {
			d->objects.erase(objIt);
		}
		object->deleteLater();
		return nullptr;
	}
	...
	if (QQuickItem *item = qobject_cast<QQuickItem *>(object)) {
		emit initItem(currentIndex, item);
		emit createdItem(currentIndex, item);
	}
