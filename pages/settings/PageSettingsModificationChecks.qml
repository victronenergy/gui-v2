...
Page {
	...
	function getSystemHooksState() {
		...
		} else if ((systemHooksState & 16) && (systemHooksState & 4) && (systemHooksState & 8)) {
			//% "Yes (rc.local and rcS.local)"
			return qsTrId("pagesettingsmodificationchecks_yes_rc_local_rcS_local")
		} else if ((systemHooksState & 16) && (systemHooksState & 8)) {
			//% "Yes (rcS.local)"
			return qsTrId("pagesettingsmodificationchecks_yes_rcS_local")
		} else if ((systemHooksState & 16) && (systemHooksState & 4)) {
			//% "Yes (rc.local)"
	...
	GradientListView {
		model: ObjectModel {
			...
			ListText {
				...
				secondaryLabel.color: fsModifiedState === 0 && systemHooksState < 16 && modelItem.value.indexOf("Raspberry") === -1 ? Theme.color_green : Theme.color_red
			}
			...
			ListText {
				...
				bottomContentChildren: [
					PrimaryListLabel {
						width: Math.min(implicitWidth, enableAllModifications.maximumContentWidth)
						topPadding: 0
						bottomPadding: 0
						color: Theme.color_font_secondary
						//% "State of root-fs"
						text: qsTrId("pagesettingsmodificationchecks_firmware_integrity_description")
					}
				]
			}
			...
			ListSwitch {
				...
				*/
				...
				onCheckedChanged: {
					...
					*/
					if (!restoreFirmwareIntegrityPressed && (enableAllModifications.checked &&
						!(systemHooksState & 16) && ((systemHooksState & 1) || (systemHooksState & 2))) || (!enableAllModifications.checked && (systemHooksState & 16))) {
			...
			ListButton {
				...
				bottomContentChildren: [
					PrimaryListLabel {
						...
						text: qsTrId("pagesettingsmodificationchecks_firmware_reinstall_description")
					}
				]
				...
				Component {
					...
					ModalWarningDialog {
						//% "This will disable all modifications, download and reinstall the latest available firmware.<br>Internet connectivity is required.<br>Press 'OK' to continue."
