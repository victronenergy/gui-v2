	...
	GradientListView {
		model: VisibleItemModel {
			...
			ListRadioButtonGroup {
				...
				preferredVisible: dataItem.valid
						&& (root.switchableOutput.type === VenusOS.SwitchableOutput_Type_Toggle)
						&& (root.switchableOutput.validFunctions !== (1 << VenusOS.Relay_Function_Manual))
				secondaryLabel.color: root.switchableOutput.hasValidFunction ? Theme.color_listItem_secondaryText : Theme.color_critical
				optionModel: {
					let options = []
					for (let i = 0; i <= VenusOS.Relay_Function_MaxSupportedType; i++) {
