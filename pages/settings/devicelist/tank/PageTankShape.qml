		...
		delegate: ListItem {
			...
			onClicked: {
				...
				if (pointCount >= 2 && pointDelegate.index < (pointCount - 1)) {
					...
					root._editPointDialog.sensorLevelMax = nextPoint[0]
					root._editPointDialog.volumeMax = nextPoint[1]
				} else {
					root._editPointDialog.sensorLevelMax = 99
					root._editPointDialog.volumeMax = 99
				}
				...
				if (pointCount >= 2 && pointDelegate.index > 0) {
					...
					root._editPointDialog.sensorLevelMin = prevPoint[0]
					root._editPointDialog.volumeMin = prevPoint[1]
				} else {
					root._editPointDialog.sensorLevelMin = 1
					root._editPointDialog.volumeMin = 1
				}

				root._editPointDialog.sensorLevel = pointDelegate.sensorLevel
				root._editPointDialog.volume = pointDelegate.volume
				root._editPointDialog.modelIndex = pointDelegate.index
				root._editPointDialog.open()
			}
		}
	}
