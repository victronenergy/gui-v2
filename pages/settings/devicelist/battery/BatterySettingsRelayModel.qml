...
DelegateComponentModel {
	...
	function showSetting() {
		for (var i = 0; i < arguments.length; i++) {
			if (arguments[i] === modeDC.currentValue)
				return true;
		}
		return false
	}
	...
	DelegateComponent {
		id: modeDC
		dataItem: VeQuickItem { uid: root.bindPrefix + "/Settings/Relay/Mode" }
	...
	DelegateComponent {
		dataItem: VeQuickItem { uid: root.bindPrefix + "/Relay/0/State" }
		preferredVisible: dataItem.valid
		ListSwitch {
			text: CommonWords.state
			dataItem.uid: root.bindPrefix + "/Relay/0/State"
			interactive: dataItem.valid && modeDC.dataItem.valid && modeDC.dataItem.value === 2
		}
	}
	...
	DelegateComponent {
		...
		preferredVisible: dischargeFloorLinkedToRelayDC.dataItem.valid && dischargeFloorLinkedToRelayDC.dataItem.value !== 0 && lowSocDC.preferredVisible
		PrimaryListLabel {
	...
	DelegateComponent {
		dataItem: VeQuickItem { uid: root.bindPrefix + "/Settings/Relay/FuseBlown" }
