	...
	GradientListView {
		model: VisibleItemModel {
			...
			ListQuantityGroup {
				...
				model: QuantityObjectModel {
					QuantityObject { object: batteryModuleData; key: "online" }
					QuantityObject { object: batteryModuleData; key: "offline" }
				}
				preferredVisible: details.allowsBatteryModules

				QtObject {
					id: batteryModuleData

					//: %1 = number of battery modules that are online
					//% "%1 online"
					readonly property string online: details.modulesOnline.isValid ? qsTrId("devicelist_batterydetails_modules_online").arg(details.modulesOnline.value) : "--"

					//: %1 = number of battery modules that are offline
					//% "%1 offline"
					readonly property string offline: details.modulesOffline.isValid ? qsTrId("devicelist_batterydetails_modules_offline").arg(details.modulesOffline.value) : "--"
				}
			}
