	...
	GradientListView {
		model: ObjectModel {
			...
			ListButton {
				...
				allowed: vrmPortalIdItem.lastFourDigits.length === 4
				onClicked: Global.dialogLayer.open(factoryResetConfirmationDialogComponent)

				VeQuickItem {
					id: vrmPortalIdItem
					uid: Global.venusPlatform.serviceUid + "/Device/UniqueId"
					property string vrmPortalId: vrmPortalIdItem.isValid ? vrmPortalIdItem.value : ""
					property string lastFourDigits: vrmPortalId.length >= 4 ? vrmPortalId.substr(vrmPortalId.length - 4) : ""
				}
				...
				Component {
					...
					ModalWarningDialog {
						...
						onAccepted: nodeRedFactoryResetItem.setValue(vrmPortalIdItem.lastFourDigits)
					}
				}
			}
