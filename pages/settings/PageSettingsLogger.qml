...
Page {
	...
	VeQuickItem {
		id: httpsEnabledItem
		uid: Global.systemSettings.serviceUid + "/Settings/Vrmlogger/HttpsEnabled"
	}

	// Hoisted from connectionStatus ListText for cross-DC access
	...
	GradientListView {
		...
		model: DelegateComponentModel {
			...
			DelegateComponent {
				id: vrmPortalModeDC
				dataItem: VeQuickItem { uid: Global.systemSettings.serviceUid + "/Settings/Network/VrmPortal" }
			...
			DelegateComponent {
				id: portalIdDC
				dataItem: VeQuickItem { uid: Global.venusPlatform.serviceUid + "/Device/UniqueId" }
				ListText {
					id: portalId
					//% "VRM Portal ID"
			...
			DelegateComponent {
				id: productIdDC
				dataItem: VeQuickItem { uid: Global.venusPlatform.serviceUid + "/Device/ProductId" }
				preferredVisible: portalIdDC.dataItem.valid && productIdDC.dataItem.valid
				ListLink {

					//% "VRM device registration"
					text: qsTrId("settings_vrm_device_registration")
					url: "https://vrm.victronenergy.com/login?installationIdentifier=%1&productId=%2".arg(portalIdDC.dataItem.value).arg(productId.value)
			...
			DelegateComponent {
				id: securityProfileDC
				dataItem: VeQuickItem { uid: Global.systemSettings.serviceUid + "/Settings/System/SecurityProfile" }
				preferredVisible: securityProfileDC.dataItem.value !== VenusOS.Security_Profile_Secured
				ListSwitch {
					id: useHttps
			...
			DelegateComponent {
				preferredVisible: !connectionStatusDC.preferredVisible
				ListText {
					...
					secondaryText: stringForErrorCode(httpsConnectionError.value)
				}
			}

			DelegateComponent {
				id: errorMessageDC
				dataItem: VeQuickItem { uid: root.loggerServiceUid + "/Vrm/ConnectionErrorMessage" }
				preferredVisible: !!errorMessageDC.dataItem.value
				ListText {
					//% "Error message: \n%1"
			...
			DelegateComponent {
				preferredVisible: !connectionStatusDC.preferredVisible && mqttRealtimeConnected.valid
				ListText {
					//% "Connection status (MQTT Real-time channel)"
					text: qsTrId("settings_connection_error_realtime_channel")
					secondaryText: mqttRealtimeStatus.value || ''
				}
			}

			DelegateComponent {
				preferredVisible: !connectionStatusDC.preferredVisible && mqttRpcConnected.valid
				ListText {
					//% "Connection status (MQTT RPC channel)"
					text: qsTrId("settings_connection_error_rpc_channel")
					secondaryText: mqttRpcStatus.value || ''
				}
			}
			...
			DelegateComponent {
				dataItem: VeQuickItem { uid: Global.systemSettings.serviceUid + "/Settings/Watchdog/VrmTimeout" }
			...
			DelegateComponent {
				dataItem: VeQuickItem { uid: root.loggerServiceUid + "/Buffer/ErrorState" }
