...
Page {
	...
	GradientListView {
		model: DelegateComponentModel {
			DelegateComponent {
				id: startingBatterySocDC
				dataItem: VeQuickItem { uid: BackendConnection.serviceUidForType("opportunityloads") + "/ReservationStartSoc" }
				ListQuantityField {
			...
			DelegateComponent {
				preferredVisible: startingBatterySocDC.dataItem.valid
				PrimaryListLabel {
			...
			DelegateComponent {
				ListQuantityField {
					...
					text: qsTrId("pagecontrollableloads_battery_at_or_above_x_soc")
							...
							.arg(startingBatterySocDC.dataItem.value)
