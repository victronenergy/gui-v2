...
Page {
	...
	property ObjectModel settingsModel: ObjectModel {
		ListRadioButtonGroup {
			...
			currentIndex: {
				let i
				for (i = 0; i < optionModel.length; ++i) {
					if (optionModel[i].value == dataItem.value) {
						return i
					}
				}
				return VenusOS.BriefView_CenterValue_SingleDoubleOrTripleGauge
			}
		...
		ListRadioButtonGroup {
			...
			property Instantiator deviceInstantiator: Instantiator {
			...
			property VeQuickItem systemBatteryService: VeQuickItem {
	...
	Instantiator {
		...
		delegate: ListRadioButtonGroup {
			...
			Component.onCompleted: root.settingsModel.append(gaugeDelegate)
		}
	}
