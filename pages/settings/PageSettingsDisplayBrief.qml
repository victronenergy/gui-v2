...
Page {
	...
	function _gaugeOptionModel(selectedCenterGaugeType, selectedCenterGaugeValue) {
	...
	function _addBatteryGaugeOption(optionModel, batteryId, name, caption) {
	...
	function _addTankGaugeOption(optionModel, tankName, serviceUid, caption) {
		optionModel.push({
			display: tankName,
			value: serviceUid,
			caption: caption,
	...
	function _findSelectedGaugeIndex(gaugeOptionModel, selectedCenterGaugeType, selectedCenterGaugeValue) {
		for (let i = 0; i < gaugeOptionModel.length; ++i) {
	...
	GradientListView {
		...
		delegate: ListNavigation {
			...
			onClicked: {
				const optionModel = root._gaugeOptionModel(centerGaugeType, centerGaugeValue)
				Global.pageManager.pushPage(deviceOptionsComponent, {
					levelIndex: index,
					title: text,
					optionModel: optionModel,
					currentIndex: root._findSelectedGaugeIndex(optionModel, centerGaugeType, centerGaugeValue),
				})
			}
