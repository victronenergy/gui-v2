...
FocusScope {
	...
	property bool _animateGeometry: root.animationEnabled && !!Global.pageManager && Global.pageManager.animatingIdleResize
	...
	onActiveFocusChanged: {
		if (Global.keyNavigationEnabled
			...
			const searchFromEnd = Global.mainView.swipeView.focusEdgeHint === Qt.BottomEdge
	...
	OverviewLayoutConditions {
		...
		onConditionChanged: Qt.callLater(root._resetWidgets)
	}

	Connections {
		target: Global.main
		enabled: Global.keyNavigationEnabled && root.activeFocus
	...
	AcLoadsWidget {
		...
		visible: layoutConditions.showAcLoads
		expanded: root._expandLayout
		animateGeometry: root._animateGeometry
		animationEnabled: root.animationEnabled

		Keys.onLeftPressed: root._horizontalKeyNavigation(acLoadsWidget, root._centerWidgets, root._lastFocusedCenterWidget)
	...
	Component {
		...
		EvcsWidget {
			...
			visible: layoutConditions.showEvChargers
			expanded: root._expandLayout
			animateGeometry: root._animateGeometry
			animationEnabled: root.animationEnabled
