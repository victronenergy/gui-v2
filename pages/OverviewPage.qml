...
SwipeViewPage {
	...
	readonly property var _leftWidgetOrder: [
		...
		VenusOS.OverviewWidget_Type_AcInputPriority, VenusOS.OverviewWidget_Type_AcInputOther,
		// End top widgets
		...
		VenusOS.OverviewWidget_Type_DcGenerator, VenusOS.OverviewWidget_Type_Alternator, VenusOS.OverviewWidget_Type_FuelCell, VenusOS.OverviewWidget_Type_Wind
		// End bottom widgets
	]
	...
	readonly property int _shouldResetWidgets: Global.dcInputs.model.count + Global.acInputs.activeInSource + (Global.acInputs.input1?.operational ? 1 : 0) + (Global.acInputs.input2?.operational ? 1 : 0) + (Global.system.showInputLoads ? 1 : 0) + (Global.system.hasAcOutSystem ? 1 : 0) + (Global.dcLoads.model.count === 0 || isNaN(Global.system.dc.power) ? 0 : 1) + (Global.solarChargers.model.count === 0 ? 0 : 1) + (Global.evChargers.model.count === 0 ? 0 : 1) + Global.evChargers.acInputPositionCount + Global.evChargers.acOutputPositionCount + (Global.pvInverters.model.count === 0 ? 0 : 1)
		...
		for (i = 0; i < widgets.length; ++i) {
			...
			case 4:
				if (largeOnlyWidgetCount === 1 || preferLargeWidgetCount === 1) {
					...
					if (largeOnlyWidgetCount === 1) {
						widget.size = widget.preferredSize === VenusOS.OverviewWidget_PreferredSize_LargeOnly ? VenusOS.OverviewWidget_Size_L : smallWidgetSize;
					} else {
						widget.size = widget.preferredSize === VenusOS.OverviewWidget_PreferredSize_PreferLarge ? VenusOS.OverviewWidget_Size_L : smallWidgetSize;
					}
				} else if (preferLargeWidgetCount === 2) {
					// If two prefer L size, then use M for those, and X/XS otherwise.
					widget.size = widget.preferredSize === VenusOS.OverviewWidget_PreferredSize_PreferLarge ? VenusOS.OverviewWidget_Size_M : (widgets.length === 3 ? VenusOS.OverviewWidget_Size_S : VenusOS.OverviewWidget_Size_XS);
				} else {
	...
	function resetWidgetPositions(widgets) {
		...
		const compactPageHeight = Theme.geometry_screen_height - Theme.geometry_statusBar_height - Theme.geometry_navigationBar_height;
		const compactWidgetsTopMargin = Math.max(0, (compactPageHeight - compactWidgetHeights) / Math.max(1, widgets.length - 1));
		const expandedPageHeight = Theme.geometry_screen_height - Theme.geometry_statusBar_height - Theme.geometry_overviewPage_layout_expanded_bottomMargin;
	...
	function _resetLeftWidgets() {
		...
		const acInputConfigs = [{
				...
				widgetType: VenusOS.OverviewWidget_Type_AcInputPriority
			}, {
				input: Global.acInputs.input2,
				widgetType: VenusOS.OverviewWidget_Type_AcInputOther
			},];
		if (Global.acInputs.isGridOrShore(Global.acInputs.input2) && !Global.acInputs.isGridOrShore(Global.acInputs.input1)) {
		...
		if (Global.solarChargers.model.count > 0 || Global.pvInverters.model.count > 0) {
			widgetCandidates.splice(_leftWidgetInsertionIndex(VenusOS.OverviewWidget_Type_Solar, widgetCandidates), 0, _createWidget(VenusOS.OverviewWidget_Type_Solar));
		}
	...
	function _inputConnectorAnimationMode(connectorWidget) {
		...
		const power = !connectorWidget.startWidget.quantityLabel.dataObject ? NaN : connectorWidget.startWidget.quantityLabel.dataObject.power;
		...
		if (connectorWidget.endWidget === inverterChargerWidget) {
			...
			return power > Theme.geometry_overviewPage_connector_animationPowerThreshold ? VenusOS.WidgetConnector_AnimationMode_StartToEnd : VenusOS.WidgetConnector_AnimationMode_EndToStart;
		} else if (connectorWidget.endWidget === batteryWidget) {
			// For DC inputs, positive power means energy is flowing towards battery.
			return power > Theme.geometry_overviewPage_connector_animationPowerThreshold ? VenusOS.WidgetConnector_AnimationMode_StartToEnd : VenusOS.WidgetConnector_AnimationMode_NotAnimated;
		} else {
			console.warn("Unrecognised connector end widget:", connectorWidget, connectorWidget.endWidget);
			return VenusOS.WidgetConnector_AnimationMode_NotAnimated;
		}
	}
	...
	FrameAnimation {
		...
		onTriggered: if (!limitFps || (currentFrame % 3 == 0))
	...
	Component {
		...
		SolarYieldWidget {
			...
			WidgetConnector {
				...
				animationMode: root.isCurrentPage && !isNaN(Global.system.solar.acPower) && Math.abs(Global.system.solar.acPower || 0) > Theme.geometry_overviewPage_connector_animationPowerThreshold ? VenusOS.WidgetConnector_AnimationMode_StartToEnd : VenusOS.WidgetConnector_AnimationMode_NotAnimated
			}
			WidgetConnector {
				id: dcSolarConnector
				...
				animationMode: root.isCurrentPage && !isNaN(Global.system.solar.dcPower) && Math.abs(Global.system.solar.dcPower) > Theme.geometry_overviewPage_connector_animationPowerThreshold ? VenusOS.WidgetConnector_AnimationMode_StartToEnd : VenusOS.WidgetConnector_AnimationMode_NotAnimated
			}
		}
	}
	...
	InverterChargerWidget {
		...
		WidgetConnectorAnchor {
			...
			visible: Global.acInputs.findValidSource() !== VenusOS.AcInputs_InputSource_NotAvailable || Global.pvInverters.model.count > 0
		}
		WidgetConnectorAnchor {
			id: inverterToAcLoadsAnchor
			location: VenusOS.WidgetConnector_Location_Right
			visible: inverterToAcLoadsConnector.visible
			y: inverterToAcLoadsConnector.straighten === VenusOS.WidgetConnector_Straighten_None ? defaultY : acLoadsToInverterAnchor.y
		}
		WidgetConnectorAnchor {
	...
	WidgetConnector {
		...
		animationMode: root.isCurrentPage && !isNaN(Global.system.load.ac.power) && Global.system.load.ac.power > 0 && Math.abs(Global.system.load.ac.power) > Theme.geometry_overviewPage_connector_animationPowerThreshold ? VenusOS.WidgetConnector_AnimationMode_StartToEnd : VenusOS.WidgetConnector_AnimationMode_NotAnimated
	}
	WidgetConnector {
		...
		animationMode: root.isCurrentPage && Math.abs(inverterChargerPower.value) > Theme.geometry_overviewPage_connector_animationPowerThreshold ? (inverterChargerPower.value > 0 ? VenusOS.WidgetConnector_AnimationMode_StartToEnd : VenusOS.WidgetConnector_AnimationMode_EndToStart) : VenusOS.WidgetConnector_AnimationMode_NotAnimated
	}
	...
	EssentialLoadsWidget {
		...
		WidgetConnector {
			...
			// If load power is positive (i.e. consumed energy), energy flows to load.
			animationMode: root.isCurrentPage && !isNaN(Global.system.load.acOut.power) && Global.system.load.acOut.power > 0 && Math.abs(Global.system.load.acOut.power) > Theme.geometry_overviewPage_connector_animationPowerThreshold ? VenusOS.WidgetConnector_AnimationMode_StartToEnd : VenusOS.WidgetConnector_AnimationMode_NotAnimated
		}
	}
	...
	Component {
		...
		DcLoadsWidget {
			...
			WidgetConnector {
				...
				// If load power is positive (i.e. consumed energy), energy flows to load.
				animationMode: root.isCurrentPage && !isNaN(Global.system.dc.power) && Global.system.dc.power > 0 && Math.abs(Global.system.dc.power) > Theme.geometry_overviewPage_connector_animationPowerThreshold ? VenusOS.WidgetConnector_AnimationMode_StartToEnd : VenusOS.WidgetConnector_AnimationMode_NotAnimated
			}
		}
	}
	...
	Component {
		...
		EvcsWidget {
			...
			readonly property bool connectToSplitAcLoads: visible && Global.system.showInputLoads                 // AC loads are split
			&& Global.evChargers.acInputPositionCount > 0   // AC-in position is in use
			readonly property bool connectToEssentialLoads: visible && Global.system.showInputLoads     // AC loads are split
			...
			WidgetConnector {
				...
				animateGeometry: root._animateGeometry
				animationEnabled: root.animationEnabled
				animationMode: root.isCurrentPage && ((evcsWidget.connectToCombinedAcLoads && Global.evChargers.power > Theme.geometry_overviewPage_connector_animationPowerThreshold) || (evcsWidget.connectToSplitAcLoads && Global.evChargers.acInputPositionPower > Theme.geometry_overviewPage_connector_animationPowerThreshold)) ? VenusOS.WidgetConnector_AnimationMode_StartToEnd : VenusOS.WidgetConnector_AnimationMode_NotAnimated
			}
			...
			WidgetConnector {
				...
				animateGeometry: root._animateGeometry
				animationEnabled: root.animationEnabled
				animationMode: root.isCurrentPage && Global.evChargers.acOutputPositionPower > Theme.geometry_overviewPage_connector_animationPowerThreshold ? VenusOS.WidgetConnector_AnimationMode_StartToEnd : VenusOS.WidgetConnector_AnimationMode_NotAnimated
			}
		}
	}
