...
Item {
	...
	// Auto-configure the start page after 1 minute, if needed.
	Timer {
		running: Global.systemSettings.startPageConfiguration.needsAutoConfigure
				 && root.pageManager.interactivity === VenusOS.PageManager_InteractionMode_Idle
		interval: Global.systemSettings.startPageConfiguration.autoConfigureTimeout * 1000
		onTriggered: {
			const mainPageName = root.pageManager.navBar.getCurrentPage()
			const mainPage = swipeView.getCurrentPage()
			Global.systemSettings.startPageConfiguration.autoConfigure(mainPageName, mainPage, pageStack.pageUrls)
		}
	}
