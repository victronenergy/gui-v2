...
Page {
	...
	readonly property bool _interactive: PageManager.navBar && !PageManager.navBar.hidden
	...
	LevelsPageButtonRow {
		...
		anchors {
			top: parent.top
			topMargin: _interactive ? 0 : -buttons.height
	...
	ListView {
		...
		anchors {
			top: buttons.bottom
			topMargin: root._interactive ? Theme.geometry.levelsPage.gaugesView.interactive.topMargin : Theme.geometry.levelsPage.gaugesView.fullScreen.topMargin
			bottom: parent.bottom
			bottomMargin: root._interactive ? Theme.geometry.levelsPage.gaugesView.interactive.bottomMargin : Theme.geometry.levelsPage.gaugesView.fullScreen.bottomMargin
			left: anchorCenter ? undefined : root.left
			leftMargin: anchorCenter ? 0 : Theme.geometry.levelsPage.gaugesView.leftMargin
			horizontalCenter: anchorCenter ? root.horizontalCenter : undefined
		}
		...
		delegate: LevelsPageGaugeDelegate {
			...
			Component.onCompleted: splitGauge.connect(gaugesView.onSplitGauge)
		}
	}
	MouseArea {
		id: idleModeMouseArea

		width: root._interactive ? 0 : root.width
		height: root._interactive ? 0 : root.height
		onClicked: {
	...
	Timer {
		id: idleModeTimer
		running: !_showTankGroupData
				 && PageManager.mainPageActive
				 && PageManager.navBar.currentUrl == "qrc:/pages/LevelsPage.qml"
				 && root._interactive
	...
	ListModel {
		...
		readonly property int type: LevelsPage.TankType.Fuel
	}
	ListModel {
		id: freshWaterTanks

		readonly property int type: LevelsPage.TankType.FreshWater
	}
	ListModel {
		id: wasteWaterTanks

		readonly property int type: LevelsPage.TankType.WasteWater
	}
	ListModel {
		id: liveWellTanks

		readonly property int type: LevelsPage.TankType.LiveWell
	}
	ListModel {
		id: oilTanks

		readonly property int type: LevelsPage.TankType.Oil
	}
	ListModel {
		id: blackWaterTanks

		readonly property int type: LevelsPage.TankType.BlackWater
	}
	ListModel {
		id: gasolineTanks

		readonly property int type: LevelsPage.TankType.Gasoline
	}
	ExpandedTanksView {
